  ...
  return (
    <div onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}
      className=" group relative flex min-h-115 w-full flex-col overflow-hidden rounded-3xl border
       border-cream/10 bg-[#151012] p-7 text-cream transition-all duration-500 hover:border-teal/30 md:p-8 "
    >
      <div className=" pointer-events-none absolute inset-0 opacity-[0.13] "
        style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, rgba(237,244,237,0.2) 1px, transparent 0)',
      ...
      <div className="relative z-10 flex flex-1 items-center justify-center">
        <AnimatePresence mode="wait">
          ...
          >
            <svg viewBox="0 0 640 640" className=" size-20 text-celadon drop-shadow-[0_0_25px_rgba(121,183,145,0.2)] md:size-24 " >
      ...
      <div className="relative z-10">
        <AnimatePresence mode="wait">
          ...
          >
            <p className=" mb-2 font-mono text-[10px] uppercase tracking-[0.2em] text-teal " >
              {activeItem.subtitle}
            </p>
            <h3 className=" font-heading text-3xl font-black uppercase leading-[0.9] tracking-[-0.03em] md:text-4xl " >
              {activeItem.title}
            </h3>
            <p className=" mt-4 text-xs leading-6 text-cream/45 " >
              {activeItem.detail}
            </p>
          </motion.div>
        </AnimatePresence>
          ...
          {cardHover.map((_, index) => (
            <button key={index} onClick={() => setCurrentIndex(index)} aria-label={`Mostrar capacidad ${index + 1}`}
              className={` h-1.5 rounded-full transition-all duration-300 ${ index === currentIndex 
                ? 'w-10 bg-teal' : 'w-3 bg-cream/20 hover:bg-cream/40'}
              `}
            />
          ))}
        </div>
